        ...
        if (interaction.options.getString('queuetype') === 'soloq') {
            ...
            await con.connect(function(err) {
                ...
                con.query(req, async function(err, result) {
                    ...
                    for (let i = 0; i < Object.keys(res).length; i++) {
                        ladder.addField((i + 1) + "/ " + res[i]['summoner_name'], '*' + res[i]['ranksoloq'] + ' ' + res[i]['lpsoloq'] + ' LP*', true);
                    }
            ...
            await con.connect(function(err) {
                ...
                con.query(req, async function(err, result) {
                    ...
                    for (let i = 0; i < Object.keys(res).length; i++) {
                        ladder.addField((i + 1) + "/ " + res[i]['summoner_name'], '*' + res[i]['rankflex'] + ' ' + res[i]['lpflex'] + ' LP*', true);
                    }
